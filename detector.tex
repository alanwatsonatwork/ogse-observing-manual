...
The detector has a active area of  $4096~\mbox{columns} \times 4096~\mbox{rows}$. The controller by default reads $4152~\mbox{columns} \times 4128~\mbox{rows}$ divided into two halves. 
...
\begin{itemize}
\item 
The \code{4kx4k} window has an active area of $4096~\mbox{columns} \times 4096~\mbox{rows}$, with 36 columns of underscan at the start and 20 at the end of each row and with 22 rows of underscan at the start of each column and 10 at the end giving a total size of $4152~\mbox{columns} \times 4128~\mbox{rows}$.
\item
